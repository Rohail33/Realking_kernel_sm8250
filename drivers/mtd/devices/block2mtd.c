...
void diskio_done(struct scsi_cmnd *scmd)
{
	...
	scsi_normalize_sense(scmd->sense_buffer, sizeof(struct scsi_sense_hdr),
			     &sshr);
...
int do_io(int cmd, struct block_device *bdev, const u_char *buf, u32 len,
	  u64 offset, u64 seq_num)
{
	int ret = 0;
	int nr = 0;
	int cdb_len = 0;
	...
	int nr_pages = ((u64)buf + len + PAGE_SIZE - 1) / PAGE_SIZE -
		       (u64)buf / PAGE_SIZE;
	...
	if (offset < 0xffffffff || len < 0xffff) {
		...
		cdb[1] = 0x0; //FUA
		cdb[2] = (u8)((offset & 0xff000000) >>
			      24); //LBA   logical block bufess cdb[2-5] offset
		cdb[3] = (u8)((offset & 0x00ff0000) >> 16);
		cdb[4] = (u8)((offset & 0x0000ff00) >> 8);
		cdb[5] = (u8)(offset & 0x000000ff);
		cdb[6] = 0; //group number
		cdb[7] = (u8)((((len + sector_size - 1) / sector_size) &
			       0x0000ff00) >>
			      8); //transfer number of sector   cdb[7-8]
		cdb[8] = (u8)(((len + sector_size - 1) / sector_size) &
			      0x000000ff);
		cdb[9] = 0; //control
		cdb_len = 10;
		...
		cdb[1] = 0x0; //FUA
		cdb[2] = (u8)((offset_h & 0xff000000) >>
			      24); //LBA   logical block bufess cdb[2-9] offset
		...
		cdb[10] = (u8)((((len + sector_size - 1) / sector_size) &
				0xff000000) >>
			       24); //transfer number of sector   cdb[10-13]
		cdb[11] = (u8)((((len + sector_size - 1) / sector_size) &
				0x00ff0000) >>
			       16);
		cdb[12] = (u8)((((len + sector_size - 1) / sector_size) &
				0x0000ff00) >>
			       8);
		cdb[13] = (u8)(((len + sector_size - 1) / sector_size) &
			       0x000000ff);
	...
	for (nr = 0; nr < nr_pages - 1; nr++) {
		...
		scmd->sdb.table.sgl[nr].page_link =
			(unsigned long)mtd_pages[nr]; //page buf
		scmd->sdb.table.sgl[nr].offset = 0; //buf offset
		scmd->sdb.table.sgl[nr].dma_address =
			page_to_phys(mtd_pages[nr]);
		scmd->sdb.table.sgl[nr].dma_length = PAGE_SIZE;
	}
	scmd->sdb.table.sgl[nr].length =
		((len % PAGE_SIZE) ? (len % PAGE_SIZE) : PAGE_SIZE);
	scmd->sdb.table.sgl[nr].page_link = (unsigned long)mtd_pages[nr];
	scmd->sdb.table.sgl[nr].offset = 0;
	scmd->sdb.table.sgl[nr].dma_address =
		(dma_addr_t)page_to_phys(mtd_pages[nr]);
	scmd->sdb.table.sgl[nr].dma_length =
		((len % PAGE_SIZE) ? (len % PAGE_SIZE) : PAGE_SIZE);
...
{
	int index =
		to >> PAGE_SHIFT; // block index, ufs block size = page size.
	...
	offset +=
		(lba +
		 index); //(bd_part->start_sect+ index);if oops address in lun0 is 32M, lba = 32*1024/4
